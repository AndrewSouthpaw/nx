...
function appOwnsExpo(appNodeModules: string): boolean {
  ...
  // no hoisted copy -> no duplicate to guard against
  if (!existsSync(hoistedExpo)) return false;
...
export function withNxMetro(userConfig: MetroConfig, opts: WithNxOptions = {}) {
  ...
  // `projectRoot`; SDK 53/54 need the workspace root so `originModulePath`
  // stays workspace-relative for the Nx resolver.
